...
class XboostCodex {
  ...
  start () {
    ...
    this.ready = (async () => {
      ...
      const socketMaybe = this.socketFactory(this.url)
      const socket = socketMaybe?.then ? await socketMaybe : socketMaybe
      await new Promise((resolve, reject) => {
        ...
        socket.onerror = () => this.fail(new Error('Cannot connect to Codex: check address, token and Chrome host permission'))
        socket.onclose = event => this.fail(new Error(`Codex WebSocket disconnected (${event?.code || 1006}${event?.reason ? `: ${event.reason}` : ''})`))
